      ...
      messages=[
          {
              "role":
                  "system",
              "content":
                  "Only use the functions you have been provided with. If the function does not satisfy the parameters of the input function or does not meet the description do not do anything"
          },
          {
              "role": "user",
              "content": question
...
def check_funcs(function_name, arguments):
  if function_name == 'query_photoobj':
    ...
    arguments_from_json = {
        key: value for key, value in arguments.items() if key in [
            "coordinates", "radius", "matches", "run", "rerun", "camcol", "field", "band", "timeout", "get_query_payload", "cache",
            "data_release", "show_progress"
        ]
    }
    ...
    arguments_from_json = {
        key: value for key, value in arguments.items() if key in [
            "coordinates", "radius", "matches", "plate", "fiberID", "mjd", "timeout", "get_query_payload", "data_release", "cache",
            "show_progress"
        ]
    }
    ...
    arguments_from_json = {key: value for key, value in arguments.items() if key in ["coordinates", "radius", "width", "height", "timeout"]}
    return SDSS.query_region(**arguments_from_json)

  elif function_name == 'query_crossid_async':
    arguments_from_json = {
        key: value for key, value in arguments.items() if key in ["coordinates", "radius", "timeout", "fields", "photoobj_field"]
    }
    return SDSS.query_crossid_async(**arguments_from_json)

  elif function_name == 'get_spectral_template':
    arguments_from_json = {key: value for key, value in arguments.items() if key in ["kind", "timeout", "show_progress"]}
    return SDSS.get_spectral_template(**arguments_from_json)

  elif function_name == 'query_specobj':
    arguments_from_json = {key: value for key, value in arguments.items() if key in ["plate", "mjd", "fiberID", "fields"]}
